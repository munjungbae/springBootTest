...
// 원본 이미지 표시
	...
	public String buy(Integer itemId, RedirectAttributes rttr, Authentication authentication) throws Exception {
		...
		//사용자 구매 장바구니에 (member#회원정보, item#상품정보) 를 저장.
...
//	// pictureFile.getBytes() => 모든 타입을 바이트배열로 변경
//	private String uploadFile(String originalName, byte[] fileData) throws Exception {
//		UUID uid = UUID.randomUUID();
//		// createdFileName = 랜덤으로생성된 파일명 + _ + 파일오리지날이름
//		String createdFileName = uid.toString() + "_" + originalName;
//		// 랜덤으로생성된 파일명 + _ + 파일오리지날이름 을 c:\\upload\ 경로에 복사
//		File target = new File(uploadPath, createdFileName);
//		FileCopyUtils.copy(fileData, target);
//		//return 값으로 해당 파일명을 호출
//		return createdFileName;
//	}
//
//	// 미리보기 이미지 표시
...
//		InputStream in = null;
//		ResponseEntity<byte[]> entity = null;
//		// itemId에 해당하는 getPreview값을 fileName에 저장
//		String fileName = itemService.getPreview(itemId);
//		try {
//			
//			// 파일명의 abc.jpg 중 . 을 기준으로 뒤쪽 잘라내기 
//			String formatName = fileName.substring(fileName.lastIndexOf(".") + 1);
//			// 미디어 타입을 잘라낸 jpg를 삽입하여 아래 함수 호출 후 파일타입 지정
//			MediaType mType = getMediaType(formatName);
//			HttpHeaders headers = new HttpHeaders();
//			//inputStream 에 c:\\upload\ 경로 file.separator + 파일이름 을 저장
//			in = new FileInputStream(uploadPath + File.separator + fileName);
//			if (mType != null) {
//				//getMediaType 함수에서 지정한 미디어 타입을 헤더에 포함
//				headers.setContentType(mType);
//			}
//			entity = new ResponseEntity<byte[]>(IOUtils.toByteArray(in), headers, HttpStatus.CREATED);
//		} catch (Exception e) {
